...
def process_single_molecule(args):
    ...
    try:
        ...
        return idx, sample_shapes, sample_n_o_f, None
...
def main():
    ...
    for idx, mol in tqdm(enumerate(supplier), desc="Reading molecules"):
        if mol is not None:
            ...
            if len(valid_mols) >= 2001:  # Stop after 2001 molecules
                break
    ...
    args_list = list(enumerate(valid_mols))
    ...
    for result in tqdm(pool.imap(process_single_molecule, args_list), total=len(args_list)):
        idx, shapes, n_o_f, error = result
        results.append(result)
        
        if shapes is not None:
            all_sample_shapes.append(shapes)
            all_sample_n_o_f.append(n_o_f)
        else:
            print(f"Failed to process molecule {idx}: {error}")
            
        # Save intermediate results every 1000 molecules
        if len(results) % 1000 == 0:
            intermediate_data = []
            for i in range(len(all_sample_shapes)):
                mol = valid_mols[i]
                for rot_idx in range(len(all_sample_shapes[i])):
                    intermediate_data.append({
                        'mol': valid_mols[i],
                        'shape': all_sample_shapes[i][rot_idx],
                        'protein_grid': all_sample_n_o_f[i][rot_idx],
                    })
            print(f"Saving intermediate results after {len(results)} molecules...")
            with open(os.path.join(output_dir, f'dataset_intermediate_{len(results)}.pkl'), 'wb') as fw:

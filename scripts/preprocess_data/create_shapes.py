...
def sdf_to_pdb(sdf_path, pdb_path):
    ...
    """
    from rdkit import Chem
    mol = Chem.SDMolSupplier(sdf_path)[0]  # Assumes a single molecule in the SDF
    if mol is None:
        raise ValueError("Failed to read the SDF file.")
    Chem.MolToPDBFile(mol, pdb_path)
    print(f"Converted {sdf_path} to {pdb_path}")
...
def select_largest_pocket(fpocket_output_dir):
    ...
    pocket_file = os.path.join(fpocket_output_dir, f"data/processed/all/pockets/pocket{largest_pocket}.pdb")
    return pocket_file


# File paths
data_path = 'chemprojector/models/encoder/data/training_data.pkl'
input_sdf = "data/Enamine_Rush-Delivery_Building_Blocks-US_249948cmpd_20241108.sdf"
protein_pdb = "data/processed/all/protein.pdb"
fpocket_output_dir = "protein_out"

# Step 1: Convert SDF to PDB
sdf_to_pdb(input_sdf, protein_pdb)

# Step 2: Run fpocket to detect cavities
run_fpocket(protein_pdb)

# Step 3: Select the largest pocket as the cavity
largest_pocket = select_largest_pocket(fpocket_output_dir)
cavity_pdb = largest_pocket  # Use the dynamically selected pocket file
protein_pdb_final = protein_pdb  # Use the converted protein PDB file
...
with open('data/processed/all/sample_shapes.pkl', 'wb') as fw:
    pkl.dump(sample_shapes, fw)

processed_data = [{'mol': shape} for shape in sample_shapes]
...
with open('data/processed/all/shape_dataset.pkl', 'wb') as fw:
    pkl.dump(dataset, fw)
...
def compute_fingerprint_from_mol(mol, cluster_HYD=True):
    ...
    # Instead of dense array, use sparse dict
    fp_sparse = {}
    ...
    for i, (type1, coords1) in enumerate(features):
        ...
        for type2, coords2 in features[i+1:]:
            ...
            if dist <= MAX_DIST:
                ...
                # Calculate fractional contributions to adjacent bins
                frac_high = bin_idx - bin_low
                frac_low = 1.0 - frac_high
                
                # Add fractional counts to both bins
                idx_low = channel * NUM_BINS + bin_low
                idx_high = channel * NUM_BINS + bin_high
                fp_sparse[idx_low] = fp_sparse.get(idx_low, 0) + frac_low
                if bin_high != bin_low:  # Only add to high bin if different
                    fp_sparse[idx_high] = fp_sparse.get(idx_high, 0) + frac_high
    
    # Convert to dense only when needed
    fp = np.zeros(n_channels * NUM_BINS, dtype=np.uint8)
    for idx, count in fp_sparse.items():
        fp[idx] = min(count, 255)  # uint8 cap
    return fp
...
#need to add this in allgpu: sparse_fp = csr_matrix(self._fp)
        #self._tree = BallTree(sparse_fp.toarray(), metric="manhattan", verbose=1)
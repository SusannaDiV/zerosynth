...
class FingerprintIndex:
    ...
    def process_molecule_batch(self, mol_batch, atom_stamp, debug=False):
        ...
        results = []
        for batch_idx, mol in enumerate(mol_batch):
            if mol is None:
                continue
                
            try:
                # Special handling for molecule 1328 in the current batch
                if batch_idx == 1328:  # Now checking the index within this batch
                    print(f"\nSkipping problematic molecule (batch index 1328) and filling with zeros")
                    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
                    results.append({
                        'mol': mol,
